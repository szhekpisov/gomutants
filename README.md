...
tags: ""                # build tags forwarded to the inner go list/test/build (e.g. "integration,debug")
...
| `--tags` | | | Comma-separated build tags forwarded as `-tags` to every inner `go list`/`go test`/`go build`, so mutation testing reaches code behind `//go:build` constraints (gremlins-compat) |
